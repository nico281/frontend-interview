...
import type { TodoList, TodoItem, CreateListInput, UpdateListInput, CreateItemInput, UpdateItemInput } from '../types';
...
export const getTodoLists = (): Promise<TodoList[]> =>
  api.get<TodoList[]>('/todo-lists').then(r => r.data);
...
export const deleteTodoList = (id: number): Promise<void> =>
  api.delete(`/todo-lists/${id}`).then(r => r.data);
...
export const reorderItem = (listId: number, itemId: number, newOrder: number): Promise<TodoItem> =>
  api.patch<TodoItem>(`/todo-lists/${listId}/todo-items/${itemId}`, { order: newOrder }).then(r => r.data);
